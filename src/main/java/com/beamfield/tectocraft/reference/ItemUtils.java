package com.beamfield.tectocraft.reference;
...
public class ItemUtils {
	...
	public static ItemStack consumeItem(ItemStack stack) {
		if (stack.stackSize == 1) {
			...
			if (item.hasContainerItem()) return item.getContainerItem(stack);
			return null;
		}
		stack.splitStack(1);
	...
	public static NBTTagInt getInt(ItemStack stack, String tagName) {
		NBTTagCompound tag = getItemTag(stack);
		NBTBase data = tag.getTag(tagName);
		return (data != null)? ((NBTTagInt)data) : null;
	}
